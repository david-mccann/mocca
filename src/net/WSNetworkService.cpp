...
std::unique_ptr<IConnectionListener> WSNetworkService::bind(const std::string& portString) {
    int port = TCPNetworkAddress::parsePort(portString);
    return std::unique_ptr<IConnectionListener>(new WSConnectionListener(port));
}
